...
struct PlayerInGameView: View {
   ...
   var body: some View {
      ...
      .onChange(of: isCurrentPlayer, {_, newValue in updateAnimateArrowsAnimation(isCurrentPlayer: newValue)})
   }
}
...
private extension PlayerInGameView {
   var imageView: some View {
      Image(player.image)
         ...
         .overlay(RoundedRectangle(cornerRadius: AppCornerRadius.overall.value)
            .stroke(player.type.isBot ? Color.appTheme.alternateAccent : Color.appTheme.accent, lineWidth: 0.2)).frame(width: 60, height: 60)
   }
   ...
   var currentSymbolView: some View {
      Image(
         systemName: orientation == .left ? "xmark" : "circle"
      )
      .fontWeight(.bold)
      .foregroundStyle(
         orientation == .left ? Color.appTheme.accent : Color.appTheme.alternateAccent
      )
   }
