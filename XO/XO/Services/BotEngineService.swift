...
protocol BotEngineProtocol {
   func bestMove(in board: Board, difficulty: Difficulty) -> CellCoordinate
}

struct BotEngineService: BotEngineProtocol {
   func bestMove(in board: Board, difficulty: Difficulty) -> CellCoordinate {
      switch difficulty {
         case .easy:
...
private extension BotEngineService {
   func randomMove(from board: Board) -> CellCoordinate {
      let emptyCells: [CellCoordinate] = board.enumerated().flatMap { rowIndex, row in row.enumerated().compactMap {
         collIndex, cell in
         cell == .empty ? CellCoordinate(row: rowIndex, col: collIndex) : nil
      }
      }
